...
/**
	国家地区选择组件：
	用法如下：<app-form-address-select [(bind)]="area" (bindChange)='showArea()'></app-form-address-select>

	bind：为双向绑定
	bindChnage：当area改变时执行的方法
**/

@Component({
  selector: 'app-form-address-select',
  templateUrl: './form-address-select.component.html',
  styleUrls: ['./form-address-select.component.styl']
})
export class FormAddressSelectComponent implements OnInit {
	...
	provincePage:boolean=false;
	cityPage:boolean=false;
	countryData:any='全部';
	provinceData:any='';
	provinceListPosition:any;
	cityData:any='';
	value:any='';	//input 显示选择结果
	countrySection:any='oftenUse';	//各个国家区间显示控制

	countryList:any=[
		{name:'oftenUse',country:['全部','中国','美国','法国','德国']},
		{name:'A-B',country:['阿尔巴尼亚','阿尔及利亚','阿富汗','阿根廷','阿拉伯联合酋长国','阿联酋','阿曼','阿塞拜疆','埃及','埃塞俄比亚','爱尔兰','爱沙尼亚','安道尔','安哥拉','安提瓜和巴布达','奥地利','澳大利亚','巴巴多斯','巴布亚新几内亚','巴哈马','巴基斯坦','巴拉圭','巴勒斯坦','巴林','巴拿马','巴西','白俄罗斯','百慕大','保加利亚','北塞浦路斯','贝宁','比利时','冰岛','波多黎各','波黑','波兰','波斯尼亚和黑塞哥维那','玻利维亚','伯利兹','博茨瓦纳','不丹','布基纳法索','布隆迪']},
		{name:'C-I',country:['朝鲜','赤道几内亚','丹麦','德国','多哥','多米尼加','多米尼克','多明尼加共和国','俄罗斯','厄瓜多尔','厄立特里亚','法国','法属波利尼西亚','法属圭亚那','法属南半球和南极领地','梵蒂冈','菲律宾','斐济','芬兰','佛得角','福克兰群岛','冈比亚','巴勒斯坦','刚果','刚果民主共和国','哥伦比亚','哥斯达黎加','格林纳达','格陵兰','格鲁吉亚','古巴','圭亚那','哈萨克斯坦','海地','韩国','荷兰','荷属圣马丁','黑山','洪都拉斯']},
		{name:'J-M',country:['基里巴斯','吉布提','吉尔吉斯斯坦','几内亚','几内亚比绍','加拿大','加纳','加蓬','柬埔寨','捷克','津巴布韦','喀麦隆','卡塔尔','科摩罗','科索沃','科特迪瓦','科威特','克罗地亚','肯尼亚','库克群岛','库拉索','拉脱维亚','莱索托','老挝','黎巴嫩','立陶宛','利比里亚','利比亚','列支敦士登','卢森堡','卢旺达','罗马尼亚','马达加斯加','马尔代夫','马耳他','马拉维','马来西亚','马里','马其顿','马绍尔群岛','毛里求斯','毛里塔尼亚','美国','蒙古','孟加拉','秘鲁','密克罗尼西亚联邦','缅甸','摩尔多瓦','摩洛哥','摩纳哥','莫桑比克','墨西哥']},
		{name:'N-V',country:['纳米比亚','南非','南苏丹','瑙鲁','尼加拉瓜','尼泊尔','尼日尔','尼日利亚','纽埃','挪威','帕劳','葡萄牙','日本','瑞典','瑞士','萨尔瓦多','萨摩亚','塞尔维亚','塞拉利昂','塞内加尔','塞浦路斯','塞舌尔','沙特阿拉伯','圣多美和普林西比','圣基茨和尼维斯','圣卢西亚','圣马力诺','圣文森特和格林纳丁斯','斯里兰卡','斯洛伐克','斯洛文尼亚','斯威士兰','苏丹','苏里南','所罗门群岛','索马里','索马里兰','塔吉克斯坦','泰国','坦桑尼亚','汤加','特里尼达和多巴哥','突尼斯','图瓦卢','土耳其','土库曼斯坦']},
		{name:'W-Z',country:['瓦努阿图','危地马拉','委内瑞拉','文莱','乌干达','乌克兰','乌拉圭','乌兹别克斯坦','希腊','西班牙','西撒哈拉','新加坡','新喀里多尼亚','新西兰','匈牙利','叙利亚','牙买加','亚美尼亚','也门','伊拉克','伊朗','以色列','意大利','印度','印度尼西亚','英国','约旦','越南','赞比亚','乍得','智利','中非','中国']}
	];
	...
	onBodyMouseDown(event) {
		...
		if (!(event.target.id == "inputbox" || $(event.target).parents("#inputbox").length>0)) {
			this.show=false;
			$('#areabox').hide();
	...
	provinceClick(province){
		...
		if(this.provinceData!='北京' && this.provinceData!='上海' && this.provinceData!='天津' && this.provinceData!='重庆'&& this.provinceData!='台湾'&& this.provinceData!='香港'&& this.provinceData!='澳门'){
			this.cityPage=true;
			this.provincePage=false;
			for(let item in this.provinceList){
				if(this.provinceList[item].name==province){
					this.provinceListPosition=item;
				}
			}
		}
		else{
		...
		this.value=this.countryData+' . '+this.provinceData;
		this.bind={
			'country':this.countryData,
			'province':this.provinceData,
			'city':''
	...
	cityClick(city){
		this.cityData=city;
		this.value=this.countryData+' . '+this.provinceData+' . '+this.cityData;
		this.bind={
			'country':this.countryData,
			'province':this.provinceData,
			'city':this.cityData
